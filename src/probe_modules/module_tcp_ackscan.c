...
// probe module for performing TCP ACK scans
...
probe_module_t module_tcp_ackscan;
static uint32_t num_ports;

static int ackscan_global_initialize(struct state_conf *state)
{
    num_ports = state->source_port_last - state->source_port_first + 1;
    return EXIT_SUCCESS;
}

static int ackscan_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
                                  port_h_t dst_port,
...
static int ackscan_make_packet(void *buf, UNUSED size_t *buf_len,
...
// not static because used by ack scan
void ackscan_print_packet(FILE *fp, void *packet)
{
...
static int ackscan_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void ackscan_process_packet(const u_char *packet,
...
probe_module_t module_tcp_ackscan = {
        ...
        .pcap_snaplen = 96,
        .port_args = 1,
        .global_initialize = &ackscan_global_initialize,
        .thread_initialize = &ackscan_init_perthread,
        .make_packet = &ackscan_make_packet,
        .print_packet = &ackscan_print_packet,
        .process_packet = &ackscan_process_packet,
        .validate_packet = &ackscan_validate_packet,
        .close = NULL,
        .helptext = "Probe module that sends a TCP ACK packet to a specific "
                "port. Possible classifications are: rst and null. A "
                "RST packet is considered a success and a ack packet "
                "is considered a failed response or filtered.",
